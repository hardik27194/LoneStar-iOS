...
#import "MoAnimatedGIFUtility.h"

@implementation MoSimpleImageDownloader
...
+ (UIImage *)imageFromData:(NSData *)data forURL:(NSURL *)imageURL canonicalURL:(NSURL *)canonicalURL {
    ...
    if (data) {
        ...
        if ([MoAnimatedGIFUtility imageURLIsAGIF:referenceURL]) {
            image = [MoAnimatedGIFUtility animatedImageWithAnimatedGIFData:data];
        }
        if (image == nil) {
