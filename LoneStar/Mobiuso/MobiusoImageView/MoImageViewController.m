...
#import "MoSimpleImageDownloader.h"
#import "UIImage+ImageEffects.h"
#import "UIApplication+MoImageViewController.h"
#import "MobiusoToast.h"

// Set up if you want the next and previous images transitioned
#define TRANSITION_STYLE_UPON_DELTA 1
#define TANSITION_STYLE_UPON_STOP   0
...
// Public Constants
CGFloat const MoImageViewController_DefaultAlphaForBackgroundDimmingOverlay = 0.66f;
CGFloat const MoImageViewController_DefaultBackgroundBlurRadius = 2.0f;
...
@interface MoImageViewController ()
<
...
// General Info
@property (strong, nonatomic, readwrite) MoImageInfo *imageInfo;
@property (strong, nonatomic, readwrite) UIImage *image;
@property (assign, nonatomic, readwrite) MoImageViewControllerTransition transition;
@property (assign, nonatomic, readwrite) MoImageViewControllerMode mode;
@property (assign, nonatomic, readwrite) MoImageViewControllerBackgroundOptions backgroundOptions;
...
@implementation MoImageViewController
...
- (instancetype)initWithImageInfo:(MoImageInfo *)imageInfo
    ...
    if (self) {
        ...
        if (_mode == MoImageViewControllerMode_Image) {
...
- (void)showFromViewController:(UIViewController *)viewController
    ...
    if (self.mode == MoImageViewControllerMode_Image) {
        if (transition == MoImageViewControllerTransition_FromOffscreen) {
    ...
    } else if (self.mode == MoImageViewControllerMode_AltText) {
...
- (void)dismiss:(BOOL)animated {
    ...
    if (self.mode == MoImageViewControllerMode_AltText) {
        [self dismissByExpandingAltTextToOffscreenPosition];
    }
    else if (self.mode == MoImageViewControllerMode_Image) {
        
        if (_flags.imageIsFlickingAwayForDismissal) {
            [self dismissByCleaningUpAfterImageWasFlickedOffscreen];
        }
        else if (self.transition == MoImageViewControllerTransition_FromOffscreen) {
            ...
            BOOL useCollapsingThumbnailStyle = (startingRectForThumbnailIsNonZero
                                                && self.image != nil
                                                && self.transition != MoImageViewControllerTransition_FromOffscreen);
...
- (void)viewDidLoad {
    ...
    miniToast = [[MobiusoToast alloc] initWithDuration: 1.0f
                                                             andText: @""];
...
#pragma mark - Image
- (void)updateImage: (PHAsset *) asset resultHandler: (void (^)(NSData *imageData,
                                                                NSString *dataUTI,
                                                                ...
                                                                NSDictionary *info))resultHandler
{
    ...
    ^(double progress,NSError *error,BOOL* stop, NSDictionary* dict) {
...
- (void)setupImageAndDownloadIfNecessary:(MoImageInfo *)imageInfo {
    if (imageInfo.image) {
        self.image = imageInfo.image;
    }

    else if (imageInfo.imageAsset) {

        self.image = imageInfo.placeholderImage;
        typeof(self) __weak weakSelf = self;
        [self updateImage:imageInfo.imageAsset resultHandler: ^(NSData *imageData,
                                                 NSString *dataUTI, UIImageOrientation orientation,  NSDictionary *info) {
            if (imageData) {
                UIImage *image = imageInfo.image = [UIImage imageWithData:imageData];
                typeof(self) strongSelf = weakSelf;
                if (image) {
        ...
        typeof(self) __weak weakSelf = self;
        NSURLSessionDataTask *task = [MoSimpleImageDownloader downloadImageForURL:imageInfo.imageURL canonicalURL:imageInfo.canonicalImageURL completion:^(UIImage *image) {
            typeof(self) strongSelf = weakSelf;
...
- (void)showImageViewerByExpandingFromOriginalPositionFromViewController:(UIViewController *)viewController {
    ...
    if (self.backgroundOptions & MoImageViewControllerBackgroundOption_Blurred) {
    ...
    [viewController presentViewController:self animated:NO completion:^{
        ...
        __weak MoImageViewController *weakSelf = self;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [UIView
                 ...
                 animations:^{
                     ...
                     CGFloat scaling;
                     if (!(weakSelf.backgroundOptions & MoImageViewControllerBackgroundOption_Scaled)) {
                         scaling = 1.0;
                     } else {
                     ...
                     if (weakSelf.backgroundOptions & MoImageViewControllerBackgroundOption_Blurred) {
...
- (CGFloat)alphaForBackgroundDimmingOverlay {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(alphaForBackgroundDimmingOverlayInImageViewer:)]) {
        ...
        alpha = MoImageViewController_DefaultAlphaForBackgroundDimmingOverlay;
    }
...
- (CGFloat)backgroundBlurRadius {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(backgroundBlurRadiusForImageViewer:)]) {
        ...
        blurRadius = MoImageViewController_DefaultBackgroundBlurRadius;
    }
...
- (void)updateLayoutsForCurrentOrientation {
    ...
    if (_flags.rotationTransformIsDirty) {
        ...
        if (_flags.isPresented) {
            ...
            CGFloat targetScaling;
            if (!(self.backgroundOptions & MoImageViewControllerBackgroundOption_Scaled)) {
                targetScaling = 1.0;
            } else {
                targetScaling = JTSImageViewController_MinimumBackgroundScaling;
            }
...
- (void)updateScrollViewAndImageViewForCurrentMetrics {
    ...
    BOOL usingOriginalPositionTransition = (self.transition == MoImageViewControllerTransition_FromOriginalPosition);
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    // Allow Flick even when slightly zoomed
    if (
#ifdef ONLYWHENZOOMEDOUT
        (scrollView.zoomScale == 1)
#else
        (scrollView.zoomScale <= JTSImageViewController_TargetZoomForDoubleTap)
#endif
        // NOTE - If you don't want the flick on horizontal side, increase the threshold for velocity.x
        && (fabs(velocity.x) > 2000 || fabs(velocity.y) > 1600 ) ) {
        DLog(@"Velocity: %4.1f, %4.1f", velocity.x, velocity.y);
        [self dismiss:YES];
    }
}
...
- (void)dismissingPanGestureRecognizerPanned:(UIPanGestureRecognizer *)panner {
    ...
    if (self.imageLocked) {
        [MobiusoToast toast:@"Sorry, the image is locked." inView:self.view];
        return;
    }
...
#if TRANSITION_STYLE_UPON_DELTA // Allow switching to next or previous
      //            CGRect f = self.imageView.frame;
            BOOL swapImage = NO;
            NSString *message;
            if (!imageChanged) {
                CGFloat delta = newAnchor.x - self.imageDragStartingPoint.x;
#ifdef SWIPE_DEBUG
                NSLog(@"New Anchor: (%3.1f, %3.1f) - starting (%3.1f, %3.2f) - delta (%3.1f)", newAnchor.x, newAnchor.y,
                      self.imageDragStartingPoint.x, self.imageDragStartingPoint.y, delta);
#endif
                if (delta < -80) {
                    if (self.dismissalDelegate && [self.dismissalDelegate respondsToSelector:@selector(imageViewController:previousImage:)]) {
                        swapImage = [self.dismissalDelegate imageViewController:self previousImage: _imageInfo.imageMarkerIndex];
                        imageChanged = YES;
                        message = @"Previous";
//                    } else {
//                        
//                        swapImage = [UIImage imageNamed:@"bkg2"];
                    }
                } else if (delta > 80) {
                    if (self.dismissalDelegate && [self.dismissalDelegate respondsToSelector:@selector(imageViewController:nextImage:)]) {
                        swapImage = [self.dismissalDelegate imageViewController:self  nextImage:_imageInfo.imageMarkerIndex];
                        imageChanged = YES;
                        message = @"Next";
//                    } else {
//                        
//                        swapImage = [UIImage imageNamed:@"bkg1"];
                    }
                }
                if (swapImage) {
//                    [miniToast setLabel: message];
//                    [miniToast displayInView: self.view];

//                    [self updateInterfaceWithImage:swapImage];
                }
            }
#endif
        } else {
...
#if TRANSITION_STYLE_UPON_STOP
            CGRect f = self.imageView.frame;
#ifdef SWIPE_DEBUG
            NSLog(@"Cancelling (%3.1f, %3.1f) : (%3.1f, %3.1f) ", f.origin.x, f.origin.y, f.size.width, f.size.height);
            UIImage *swapImage = nil;
#endif
            if (f.origin.x > 100) {
                if (self.dismissalDelegate && [self.dismissalDelegate respondsToSelector:@selector(previousImage)]) {
                    swapImage = [self.dismissalDelegate previousImage];
                    
                } else {
                    
                    swapImage = [UIImage imageNamed:@"neetasandeep.jpg"];
                }
            } else if (f.origin.x < -100) {
                if (self.dismissalDelegate && [self.dismissalDelegate respondsToSelector:@selector(nextImage)]) {
                    swapImage = [self.dismissalDelegate nextImage];
                    
                } else {
                    
                    swapImage = [UIImage imageNamed:@"banecat-red.png"];
                }
            }
            if (swapImage) {
                [self updateInterfaceWithImage:swapImage];
                ...
                [UIView transitionWithView:self.imageView
                                  duration:0.4f
                                   options:UIViewAnimationOptionTransitionCrossDissolve
                                animations:^{
                                    self.imageView.image = swapImage;
                                } completion:nil];
#endif
            }
#endif
