...
{
	return [NSString stringWithFormat:@"<%@ 0x%@ type=%u position=%u>", NSStringFromClass([self class]), self, (unsigned int)_type, (unsigned int)_position];
}
...
- (NSString *)prefixWithCounter:(NSInteger)counter
{
	...
	switch (listStyleType) 
	{
		...
		{
			token = [NSString stringWithFormat:@"%ld.", (long)counter];
			break;
		}
		case DTCSSListStyleTypeDecimalLeadingZero:
		{
			token = [NSString stringWithFormat:@"%02ld.", (long)counter];
			break;
		}
