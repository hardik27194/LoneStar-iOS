...
#import "MoElasticMenuOverlayView.h"
#import <QuartzCore/QuartzCore.h>
#import "Constants.h"
#import "NSString+StringSizeWithFont.h"

@interface MoElasticMenuOverlayView()
-(void)backtonormal;
-(void)configureView;
...
@implementation MoElasticMenuOverlayView
...
- (id)initWithFrame:(CGRect)frame withDelegate:(id <MoElasticMenuDelegate>)del
{
...
-(void)setBubbleText:(NSString *)tempString
{
    ...
    CGSize tempSize = [tempString sizeWithFontSafe:tempFont];
...
-(void)configureView
{
    ...
    for(int i = 0;i<self.controlsCount;i++)
    {
        ...
        tempImage = [self.delegate menuItemImageForIndex:i];
        ...
        tempHighlightImage = [self.delegate menuItemHighlightImageForIndex:i];
        tempHighlightLayer.contents = (id)tempHighlightImage.CGImage;
        
        tempName = [self.delegate menuItemTitleForIndex:i];
...
- (void) configureHint
{
    _elasticMenuPopImage = [[UIImageView alloc] initWithFrame: CGRectMake(self.frame.size.width-60, -32, 64, 64)];
...
#define ELASTIC_MENU_DELTA  5
    ...
    // notused CGFloat currentControlHeight = self.maxHeight;
    ...
    // notused CALayer *currentLayer1 = [self.controlsFrameArray objectAtIndex:0];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        [self.delegate didSelectMenuItem:self.currentIndex];
    }
