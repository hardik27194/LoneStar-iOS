...
{
    [self setScrollViewContents: views];
...
{
    [self changeToPage:self.pageControl.currentPage];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // DLog(@"pagewidth=%.f x=%.f = position=%.f", pageWidth, scrollView.contentOffset.x, ((CALayer *)[[self.layer sublayers] objectAtIndex:0]).position.x);
    // The following provides the Parallax effect (Sandeep - 20150109)
    CGPoint position = ((CALayer *)[[self.layer sublayers] objectAtIndex:0]).position;
    
#ifdef OLD
    ((CALayer *)[[self.layer sublayers] objectAtIndex:0]).position = CGPointMake(pageWidth/2 - scrollView.contentOffset.x/20, position.y);
#else
    ...
    // DLog(@"Jumping -->>>> %.f\nPage: %d\n\n", scrollView.contentOffset.x, page);

    if (self.pageControl.currentPage != page) {
        // call the delegate
        if ([delegate respondsToSelector:@selector(pageChanged:)]) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if ([delegate respondsToSelector:@selector(scrollEnded)]) {
...
{
    self.pageControlIsChangingPage = NO;
    if ([delegate respondsToSelector:@selector(scrollStarted)]) {
