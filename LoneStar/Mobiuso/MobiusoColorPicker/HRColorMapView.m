...
+ (UIImage *)colorMapImageWithSize:(CGSize)size
    ...
    // We need to center horizontally and set it at the bottom if we can't get the correct multiples..
    CGPoint origin = CGPointMake( (size.width - (pixelCountX * tileSize)) / 2, (size.height - (pixelCountY * tileSize)));
    ...
    return [UIImage hr_imageWithSize:colorMapSize rectOffset:origin opaque:NO renderer:renderToContext];
}
...
- (void)handlePan:(UIPanGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        if (_colorMapViewDelegate && [_colorMapViewDelegate respondsToSelector:@selector(touchesBegan)]) {
            [_colorMapViewDelegate touchesBegan];
        }
    } else if (sender.state == UIGestureRecognizerStateEnded) {
        if (_colorMapViewDelegate && [_colorMapViewDelegate respondsToSelector:@selector(touchesEnded)]) {
            [_colorMapViewDelegate touchesEnded];
        }
    }
