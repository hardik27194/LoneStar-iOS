...
static HomeViewController *myself;

@interface HomeViewController () <BookPortalViewDelegate, UIWebViewDelegate, UIViewControllerTransitioningDelegate
#if 0
...
#ifndef SPECIFIC_PRODUCT
@property (nonatomic, retain) NSTimer *fakeProgressTimer;
@property (nonatomic, assign) NSInteger fakeProgressValue;
...
#ifndef SPECIFIC_PRODUCT
    // Show some fake progress view
    if (_fakeProgressTimer) {
        [_fakeProgressTimer invalidate];
    }
    _fakeProgressValue = 0;
    _fakeProgressTimer = [NSTimer scheduledTimerWithTimeInterval:1.0  target:self selector:@selector(fakeProgress)  userInfo:nil repeats:NO];
...
- (void) fakeProgress
{
    ...
    if (_fakeProgressValue <= 100 ) {
        _fakeProgressValue += arc4random()%4;   // add some random value
        [_ipv setProgress: (_fakeProgressValue % 100)];
...
- (void)didPressInfo {
    ...
    UIImage *aboutPage = [HomeViewController screenshot: self.titleLabel.text size:self.view.bounds.size posterImage:nil category:self.artistLabel.text];
...
- (void) actionThreeButtonTapped: (UIButton *) sender isSelected: (BOOL) selected
{
    ...
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName: @"TOC" bundle:nil];
    
    
    TOCMainViewController *controller = [storyboard instantiateViewControllerWithIdentifier:@"TOCViewController"];
...
- (void) didPressSettings: (id) sender
{
    ...
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName: @"Galaxy" bundle:nil];
    
    // Test
    //    NotesAccountController *controller = [[NotesAccountController alloc] init];
    
    GalaxyViewController *controller = [storyboard instantiateViewControllerWithIdentifier:@"GalaxyViewController"];
    
    
    controller.transitioningDelegate = self;
//    self.transitionType = MOGalaxyTransition;
    controller.modalPresentationStyle = UIModalPresentationCustom;
#if 0
    Mixpanel *mixpanel = [Mixpanel sharedInstance];
    [mixpanel identify:[AppDelegate getDistinctIdForMixpanel]];
    [mixpanel.people increment:@"Galaxy accessed count" by:@1];
#endif
