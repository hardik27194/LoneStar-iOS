...
@objc protocol BookPortalViewDelegate {
...
@IBDesignable
@objc class BookPortalView : UIView {
    
    var view: UIView!
    var delegate: BookPortalViewDelegate?
    ...
    // If we are jumping around on the animation, we need to adjust it (if we are paused)
    ...
    private func loadViewFromNib() -> UIView {
        ...
        let nib = UINib(nibName: "BookPortalView", bundle: bundle)
        
        // Assumes UIView is top level and only object in CustomView.xib file
        let view = nib.instantiateWithOwner(self, options: nil)[0] as! UIView
        
    
        actionOne_icon_selected = UIImage.RemapColor(UIColor.whiteColor(), maskImage: actionOne_icon_selected)
        actionOne_icon_unselected = UIImage.RemapColor(UIColor.whiteColor(), maskImage: actionOne_icon_unselected)
        return view
    }
    ...
    private func resumeLayer(layer : CALayer) {
        ...
        layer.beginTime = timeSincePause - delta
    }
    ...
    func updateTime(){
        ...
        progressLabel.text = NSString(format: "%i:%02i",min,sec ) as String
    ...
    func start(){
        if !self.isAnimating {
            self.animateCircle(songDuration)
            self.startTimer()
        }else {
    ...
    func seekToTime(newTime: Double) {
